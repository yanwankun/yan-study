...
import cn.yan.study.websocket.mychat.common.utils.StringUtil;
...
import cn.yan.study.websocket.mychat.task.MsgAsyncTesk;
...
public class TextWebSocketFrameHandler extends SimpleChannelInboundHandler<Object>{
    ...
    private void textWebSocketFrame(ChannelHandlerContext ctx, TextWebSocketFrame msg) {
        ...
        //用户登录判断
        MyChatMsg myChatMsg = JSON.parseObject(rMsg, MyChatMsg.class);
        if (redisTemplate.check(incoming.id(),rName)){
            //临时存储聊天数据
            cacheTemplate.save(myChatMsg);
            //存储随机链接ID与对应登录用户名
